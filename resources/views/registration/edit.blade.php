        ...
        <div class="card mt-4">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('aircraft_types.registrations.show', ['aircraft_type' => $registration->aircraftType, 'registration' => $registration->id]) }}"
                        class="btn btn-secondary btn-sm bi-backspace float-end mt-0"> Back</a>
                </div>
            </div>
            <div class="card-body">
                <form class="row"
                    action="{{ route('aircraft_types.registrations.update', ['aircraft_type' => $registration->aircraftType, 'registration' => $registration->id]) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-6 mb-3">
                        ...
                        <input type="number" name="basic_weight" id="basic_weight"
                            ...
                            value="{{ $registration->basic_weight }}">
                        @error('basic_weight')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        <button type="submit" class="btn btn-primary bi-floppy-fill float-end"> Add Registration</button>
                    </div>
                </form>
            </div>
        </div>
