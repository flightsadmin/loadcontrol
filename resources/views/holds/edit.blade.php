            ...
            <div class="card-body">
                <form action="{{ route('holds.update', $hold->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="hold_no" name="hold_no" class="form-control" value="{{ old('hold_no', $hold->hold_no) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="fwd" class="form-label">Forward Distance</label>
                        <input type="number" id="fwd" name="fwd" class="form-control" value="{{ old('fwd', $hold->fwd) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="aft" class="form-label">Aft Distance</label>
                        <input type="number" id="aft" name="aft" class="form-control" value="{{ old('aft', $hold->aft) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="max" class="form-label">Maximum Weight</label>
                        <input type="number" id="max" name="max" class="form-control" value="{{ old('max', $hold->max) }}" required>
                    </div>
